 ...
 *
 * var perf = helpers.execTime('op1');
 * // ...
 *
 * log.info('Total execution time:' + perf());
 *
 *
 * @param  {string} id The unique identifier.
 * @return {Function} Call it to get the result.
 */
helpers.execTime = function( id ) {

  if (_timers[id]) {
    return _timers[id].fn;
  }
  ...
  _timers[id] = {
    /**
     * @return {string} formated time diff
     */
    fn: function() {
      var diffObj = moment.duration(moment() - start);

      return diffObj.minutes() + 'm ' + diffObj.seconds() + 's ' +
        diffObj.milliseconds() + 'ms';
    }
  };

  _timers[id].fn.reset = function() {
    delete _timers[id];
  };
